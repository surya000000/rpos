...
if [ ! -z `command -v ldconfig` ]; then
    if [ `ldconfig -p | grep -w -i libxext.so | wc -l` -eq 0 ]; then
        echo "Missing dependent library libXext."
        exit 1
    fi
    if [ `ldconfig -p | grep -w -i libxi.so | wc -l` -eq 0 ]; then
        echo "Missing dependent library libXi."
        exit 1
    fi
    if [ `ldconfig -p | grep -w -i libxtst.so | wc -l` -eq 0 ]; then
        echo "Missing dependent library libXtst"
        exit 1
    fi
    if [ `ldconfig -p | grep -w -i libxrender.so | wc -l` -eq 0 ]; then
...
case "$1" in
    run)
        ...
        if [ ! -f $PID_PATH_NAME ] || [ `ps -axo pid | grep -w "$(cat $PID_PATH_NAME)" | wc -l` -eq 0 ]; then
    ...
    stop)
        if [ -f $PID_PATH_NAME ]; then
            ...
            sleep 1
            if [ `ps -axo pid | grep -w "$(cat $PID_PATH_NAME)" | wc -l` -eq 0 ]; then
